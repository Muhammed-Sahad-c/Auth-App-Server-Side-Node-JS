...
export const authControllers = {
  ...
  signupDataSubmission: async (req, res, next) => {
    ...
    try {
      ...
      if (!isAlreadyUser) {
        ...
        res
          .status(200)
          .json({ status: true, message: `otp sen't to your account` });
      } else {
  ...
  loginDataAuthentication: async (req, res, next) => {
    try {
      ...
      res.json({ status: false, message: `OOPS! try again!` });
    }
  },
    ...
    try {
      ...
      if (isUser) {
        let info = await transporter.sendMail({
          ...
          html:
            "<h3>Verify your email for reset password</h3>" +
        ...
        res.json({ status: false, message: `couldn't find email try again!` });
      }
    } catch (error) {
